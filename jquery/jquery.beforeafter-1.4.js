...
(function($){
	$.fn.extend({
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="34" height="55" alt="handle" src="'+o.imagePath+'handle.svg" id="handle'+randID+'" /></div></div>'); // Create drag handle
			...
			$('#drag'+randID).width(3).height(imgHeight).css({'background':o.dividerColor,'position':'absolute','left':'17px'});	// Set drag handle CSS properties
			$('#beforeimage'+randID).css({'position':'absolute','top':'0px','left':'0px'});
			$('#afterimage'+randID).css({'position':'absolute','top':'0px','right':'0px'});
			$('#handle'+randID).css({'z-index':'100','opacity':'.85','position':'relative','cursor':o.cursor,'top':(imgHeight/2)-($('#handle'+randID).height()/2)+'px','left':'-16px'}) // aici
			...
			if(o.showFullLinks)
			{
				...
				$('#showleft'+randID).css({'position':'relative','left':'0px', 'top':'9px'}).click(function(){
				...
				$('#showright'+randID).css({'position':'absolute','right':'0px','top':'9px'}).click(function(){
			...
			function drag()
			{
				$('div:eq(2)', obj).width( parseInt( $(this).css('left') ) + 19  ); // aici sincron imagine cand tragi de handle
			}
